export const parseStorage = test => {
  const getStorageRec = ({ args }) => {
    let label = [] as any;
    for (const arg of args) {
      if (Array.isArray(arg)) {
        label.push(arg);
      } else if (arg.prim == 'Pair') {
        label = [...label, ...getStorageRec(arg)];
      } else {
        label.push(arg);
      }
    }
  ...
  return getStorageRec(test.script.storage).map(x => {
    if (Array.isArray(x)) {
      return x.reduce((prevMap, current) => {
        const key = (Object as any).values(current.args[0]);
        const val = (Object as any).values(current.args[1]);
        prevMap.set(key, val);
        return prevMap;
      }, new Map());
    }

    return (Object as any).values(x)[0];
  });
};
...
export function decodeSchema(collapsed_tree) {
  function decode_node(node) {
    ...
    if (['map', 'big_map'].indexOf(node.prim) != -1) {
      return {
        [node.prim]: [decode_node(node.args[0]), decode_node(node.args[1])]
      };
    }
...
function get_flat_nested(nested) {
  let flat_args = [] as any[];

  nested.args.forEach((arg: any) => {
    if (arg instanceof Nested && (arg as any).prim === nested.prim) {
...
function getAnnotation(x, prefix, def = undefined): any {
  const ret = [] as any;
...
function decode_literal(node, prim) {
  ...
  if (prim === 'key' && core_type === 'bytes') {
    ...
    // eslint-disable-next-line
    console.log(
      'Houston we have a problem with PREFIX: ',
      prim,
      core_type,
      value
    );
